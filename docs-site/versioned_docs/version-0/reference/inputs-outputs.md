...
| `include_codebase_context` | No | `false` | When `true`, the rest of the project is sent to the AI as background: every eligible file left once the exclusions are applied and the assessed files are set aside, meaning every file that passes the exclude patterns and didn't change in the assessed range. That is starter code the student has never changed and, when the range starts after the first commit (`tag_diff_base: previous-tag` or `tag:<name>`, or `base_sha`), the student's earlier work that this submission didn't touch. Files left out by a rule (exclude patterns, `skip_committers`, binary files) are never sent. Questions may draw on it, but a question showing only these files is dropped. See [Codebase context](code-selection.md#codebase-context) |
| `codebase_context_max_chars` | No | `50000` | Maximum total characters sent when `include_codebase_context` is `true`. Starter code and earlier work share the limit. Files are added whole, nearest to the assessed files first; a file that would exceed the limit is left out and counted in the run summary. Values below 1 are clamped to 1 |
