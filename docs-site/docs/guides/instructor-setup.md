...
![A cartoon split by a brick wall with an "Instructors only" sign and a padlock. The GrillMyCode flame sits on the wall and sends a paper plane to each side. On the student's side, the student looks at a pinned issue of questions with a PDF copy, and every answer is hidden. On your side, a locked folder with a tab for each student, such as jsmith, holds questions with answers, and a quiz card flies off to your LMS. Chips underneath compare the sides: the student gets questions but not answers; you get questions, answers and an LMS quiz.](/img/who-sees-what.svg)
