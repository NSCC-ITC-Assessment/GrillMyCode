...
| `submission_tags` | No | | **Tag-triggered workflows only.** Comma- or newline-separated tag patterns that mark a submission — list the same patterns as the workflow's `on.push.tags` (e.g. `complete, phase1`, or `submit/*` for Classroom 50 submit-only assignments). Patterns use GitHub's filter syntax: `*` (not crossing `/`), `**`, `?`, `+` and `[ ]` classes; `!` negation is not supported. A run started by a tag fails unless the tag matches one of these patterns **and** points at a commit on the default branch. Each pattern is its own delivery group — one issue, one PDF, one instructor-repository folder — so `phase1` and `phase2` are kept apart while every `submit/*` tag shares one. Ignored on runs not started by a tag. See [Tag Submission](../example-workflows/tag-submission.md) |
