...
| `exclude_pattern_overrides` | No | | Comma-separated entries to re-include files that are excluded by default. Each entry can be an exact default pattern (e.g. `**/*.md`) to re-include all files of that type, or a specific file path (e.g. `README.md`) to allow only that file through while the default pattern still excludes everything else. Note: binary files are **always** skipped regardless of overrides |
| `exclude_patterns` | No | *(common non-code files)* | Comma-separated globs for **additional** files to exclude. These are merged with the built-in default list — the defaults are always applied |
