...
| `assignment_context`           | No       |                                             | Comma-separated file glob(s) read from the repository and injected into the AI prompt before `instructor_context`. Treated as **untrusted reference data** that only steers question topics — it cannot override the rubric or reveal answers. **Security:** point this only at instructor-controlled paths; globs match the student's checked-out tree, so `README.md` / `**/*.md` can pick up student-editable files. Use `instructor_context` for instructions that must take effect. Supported file types: plain text / source files (UTF-8), PDF (`.pdf` — text layer only), Microsoft Word (`.doc`/`.docx` — text only). If no files match, a workflow warning is emitted and the action continues without context. Example: `"docs/brief.pdf, instructor/rubric.docx"`. |
