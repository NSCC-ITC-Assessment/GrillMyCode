...
| `exclude_pattern_overrides`    | No       |                                             | Comma-separated entries that allow specific files through the [default exclude list](https://nscc-itc-assessment.github.io/GrillMyCode/docs/guides/exclude-patterns#default-patterns). Each entry can be an **exact default pattern** (e.g. `**/*.md` — re-includes all Markdown files) or a **specific file path** excluded by a default pattern (e.g. `README.md` — only that file passes through while `**/*.md` still excludes everything else).                                                                                                         |
| `exclude_patterns`             | No       | _(common non-code files)_                   | Comma-separated globs for **additional** files to exclude. These are merged with the [built-in default list](https://nscc-itc-assessment.github.io/GrillMyCode/docs/guides/exclude-patterns#default-patterns) — the defaults are always applied. Supply extra patterns here to exclude files beyond the defaults.                                                                                                                                                                                                                                            |
...
The default `exclude_patterns` list covers common non-code files (lock files, images, build artefacts, minified assets, and Markdown files). This list is **always applied**. Any value supplied via `exclude_patterns` is merged with the defaults — it does not replace them. To exclude additional files on top of the defaults:
