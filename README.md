...
| `include_codebase_context`     | No       | `false`                        | Give the AI the rest of the project as background: every eligible file left once the exclusions are applied and the assessed files are set aside. That's starter code the student hasn't changed, plus their earlier work on tag runs that assess only the latest phase. Questions are never about it alone                                          |
| `codebase_context_max_chars`   | No       | `50000`                        | Maximum characters sent as codebase context. Files nearest the assessed files go first; any that don't fit are left out                                                                                                                                                                                                                              |
