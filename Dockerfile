...
#                      gitignore-templates.json), the logo embedded in
#                      the PDF, and the entrypoint shell script are copied in.
#
...
# Copy source code
COPY src/ ./src/
# The PDF heading embeds the logo; keep its repo-relative path so
