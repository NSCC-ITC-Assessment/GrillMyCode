...
Present only when the assignment's workflow sets `label_repos: "true"`. GrillMyCode labels each student repository — a `grillmycode` topic and a note in the repository description — when it generates an assessment, so you can tell which repositories have a question set from the organisation's repository list.

Nothing in the assessment run ever _clears_ a label, because the action only runs when a student pushes. `reconcile-repo-labels.yml` closes that gap: once a day it compares every repository in this assignment against the live state — an open issue labelled `assessment` is the source of truth — and adds or removes labels to match. Run it from the Actions tab with **Report what would change** ticked to preview it without writing anything.
...
It needs the same PAT as instructor delivery, read from an `INSTRUCTOR_REPO_TOKEN` secret this repository can see. An organisation-level secret visible to private repositories covers it with no per-assignment setup; without the secret the job fails with a message saying so. Description labels are cleared by the sweep but never re-added, because only the action knows the question count.
