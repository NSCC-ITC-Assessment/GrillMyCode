...
- CRITICAL — Write all four options as complete, confident explanations BEFORE deciding formatting. Each option (correct and incorrect) must read like a full answer a knowledgeable student might give. Do not write the correct answer with detail and then write throwaway short distractors. The distractors must contain specific technical claims (variable names, conditions, mechanisms) that make them sound authoritative but are factually wrong.
- MANDATORY length distribution: For at least half of all questions, the longest option must be one of the INCORRECT options — not the correct answer. Achieve this by adding a "because..." clause, a parenthetical detail, or a second sentence to at least one distractor that elaborates on its (wrong) reasoning. Example distractor format: "The loop terminates early when the condition fails, because the break statement inside the nested if block transfers control to the first statement after the outer loop (skipping the else clause entirely)." — this is longer than many correct answers would be, yet still wrong.
- MANDATORY length-verification rule: Before finalizing each question, count character lengths. Output after the full answer block: \`<!-- Lengths: C=XX | D1=XX | D2=XX | D3=XX -->\`. If the correct answer is the longest option, revise at least one distractor to be longer by adding elaboration. The correct answer must NOT be the longest option in more than one-third of questions.
